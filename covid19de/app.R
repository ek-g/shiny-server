...
ui <- dashboardPage(skin = "red",
  ...
  dashboardBody(
    ...
    fluidRow(
        column(8,
             box(plotlyOutput("landplot", height = "685px"), width = NULL)
              ),
        column(4,
               fluidRow(
                    column(6,
                        valueBox(paste0(gesamtzahl), paste0("Infizierte (", datum, ")"),
                                    icon = icon("bug"),
                                    width = NULL,
                                    color = "red")
                            ),
               ...
               valueBox(sum(todesfaelle$Tote), paste0("Todesfälle (", datum, ")"),
                        icon = icon("bug"),
                        width = NULL,
                        color = "black"),
               tabBox(
                 #title = paste("Stand:", datum),
                 tabPanel("Infizierte pro 1000", plotlyOutput("relplot"), width = NULL),
                 tabPanel("Todesfälle", plotlyOutput("toteplot"), width = NULL),
                 width= NULL)
               )
            ),
    fluidRow(column(12, 
                box(p("Letzte Aktualisierung am ", file.info(datapath)$mtime, "- Daten: ", a("Risklayer", href="http://www.risklayer.com/de/")),
...
server <- function(input, output) {
    ...
    output$relplot <- renderPlotly({
        ...
        landdata %>% 
            ...
            mutate(pro1000 = Infizierte / Bevoelkerung *1000) %>% 
            arrange(pro1000) %>% 
            ...
            layout(yaxis = list(categoryorder = "array", categoryarray = ~pro1000,
                                title = "Bundesland"),
                   xaxis = list(title = "Infizierte pro 1000 Einwohner")) %>%
            hide_colorbar()
    })
        ...
        todesfaelle %>% 
            ...
            plot_ly(x = ~Tote, y = ~Kuerzel,
                    ...
                    hovertemplate = paste(
                        ...
                        "%{x:d} Todesfälle<br>",
                        "<extra></extra>")) %>% 
            layout(yaxis = list(categoryorder = "array", categoryarray = ~Tote,
                                title = "Bundesland"),
                   xaxis = list(title = "Todesfälle pro Bundesland")) %>%
            hide_colorbar()
    })
    
    output$datatable = renderDT(
      landdata %>% select(-Bevoelkerung), options = list(lengthChange = FALSE)
    )
}
